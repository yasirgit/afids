...
<h2>Person Find :: Requester Add</h2>
<div class="missions-available">
  <form id="filter_form" method="post" action="<?php echo url_for('@requester_find')?>">
      ...
      <div class="missions-available-filter">
        <div class="bg">
          <div class="characteristic_clean">
            <div class="holder">
              <div>
                ...
                <?php echo input_auto_complete_tag('firstname', $firstname == '*' ? '' : $firstname,
                                      'requester/autoCompleteFirstNon',
            ...
            <?php echo link_to('reset', 'requester/find?filter=1')?>
          </div>
...
<div class="pager">
  ...
  <div>
    <form action="<?php echo url_for('@requester_find?module=requester')?>">
      <?php echo link_to('Previous', '@requester_find?module=requester&page='.$pager->getPreviousPage(), array('class' => 'btn-pager-prev'))?>
      <input type="text" name="page" class="active-page" value="<?php echo $pager->getPage()?>"/>
      <strong>of <?php echo link_to($pager->getLastPage(), '@requester_find?module=requester&page='.$pager->getLastPage())?></strong>
      <?php echo link_to('Next', '@requester_find?module=requester&page='.$pager->getNextPage(), array('class' => 'btn-pager-next'))?>
  ...
  <tr>
    ...
    <td>
      <ul class="action-list">
          <li><?php if ($sf_user->hasCredential(array('Administrator','Staff','Pilot','Coordinator','Volunteer'), false)):?><a class="action-view" href="<?php echo url_for('@person_view?id='.$person->getId())?>">View</a><?php endif;?></li>
      </ul>
    </td>
  </tr>
  <?php endforeach;?>
