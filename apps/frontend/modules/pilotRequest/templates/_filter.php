...
<form action="<?php echo url_for($action_url)?>" id="form_filter" method="post">
  <fieldset>
...
<?php //$pager->setNbResults(20); echo $pager->getNbResults(200); $pager->setLastPage(2);?>
<fieldset>
  <div class="sort">
    <label for="sort_by">Sort by:</label>
    <form method="post" action="<?php echo url_for($action_url)?>">
    ...
    <div class="pager">
      <form action="<?php echo url_for($action_url)?>">
        <input type="hidden" name="filter" value="true">
        <?php echo link_to('Previous', $action_url.'?page='.$pager->getPreviousPage().'&filter=true', array('class' => 'btn-pager-prev'))?>
        <input type="text" name="page" class="active-page" value="<?php echo $pager->getPage()?>"/>
        <strong>of <?php echo link_to($pager->getLastPage(), $action_url.'?page='.$pager->getLastPage().'&filter=true')?></strong>
        <?php echo link_to('Next', $action_url.'?page='.$pager->getNextPage().'&filter=true', array('class' => 'btn-pager-next'))?>
