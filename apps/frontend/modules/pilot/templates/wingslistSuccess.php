  ...
  <form action="#" id="filter_email_form">
    <fieldset>
      <div class="box full">
         ...
         $("#efficiency").bind('change',function(event){
          var getVlue = jQuery("#efficiency").val();
            if((/^\d{1,3}$/.test(getVlue))){
             $('#efficiency').val(getVlue);
            }else {
              alert("Efficiency must be digit");
              jQuery("#efficiency").val('');
            }
            if(parseInt(getVlue, 10) > 100 ){
                alert("Input can not be grater than 100");
                jQuery("#efficiency").val('');
            }             
           });
          
          $("#zipcode").bind('change',function(event){
          var getVlueZip = jQuery("#zipcode").val();         
          if(/^\d{5}([\-]\d{4})?$/.test(getVlueZip)){               
                $('#zipcode').val(getVlueZip);
            }else{
               alert("Zipcode does not match, format should be 92648-3773 or 98040");
               jQuery("#zipcode").val('');
            }
           });
          });           
        ...
        <div class="wrap">
          ...
          <input type="text" class="text narrowest" id="area_code" size="3" name="area_code" onchange="countFiltered('filter_email_form');"/>
           eg: 562,206 (2 or 3 digits)
        </div>
        <div class="wrap">
        ...
        <div class="wrap">
          ...
          <input type="text" class="text narrowest" id="zipcode" name="zipcode" onchange="countFiltered('filter_email_form');"/>
          eg: 92648-3773 or 98040
        </div>
        <!--div class="wrap">
          <label for="active">Pilot Status</label>
          <input type="text" class="text narrowest" id="active" name="active" onchange="countFiltered('filter_email_form');"/>
        </div-->
