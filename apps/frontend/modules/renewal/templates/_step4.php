...
<p style="padding: 12px 30px 0px 10px;font-size:12px;font-family:Arial,Helvetica,sans-serif">I understand that it is my responsibility and obligation to decline serving as pilot-in-command on any flight referred to me by Angel Flight West unless ALL of the above statements are true at the time of the flight.  I further understand that I will complete this affirmation annually at the time of my membership renewal.</p>
    ...
    <form action="<?php echo url_for('renewal/step4Check?id='.$application_temp->getId()) ?>" method="post">
      <div class="box full">
        <div class="wrap">
          <?php echo $widget->render('affirmation_agreed', ESC_RAW)?>
        </div>
        <div class="form-submit">
          <a class="btn-action" href="#" onclick="$('#step4_submit').click(); return false;"><span>Save and Continue »</span><strong> </strong></a>
          <a class="cancel" href="#" onclick="$('<?php echo $application_temp->getApplicantPilot()?'.third':'.second'?>').click();">&laquo; Back</a>
          <input type="submit" id="step4_submit" value="submit" class="hide"/>
        </div>
      </div>
    </form>
  </div>
