...
/**
 * Base class that represents a row from the 'person_role' table.
 *
 * 
 *
 ...
 */
abstract class BasePersonRole extends BaseObject  implements Persistent {


  const PEER = 'PersonRolePeer';
	...
	public function setPersonId($v)
	{
		...
		if ($this->person_id !== $v) {
			$this->person_id = $v;
			$this->modifiedColumns[] = PersonRolePeer::PERSON_ID;
		}
	...
	public function setRoleId($v)
	{
		...
		if ($this->role_id !== $v) {
			$this->role_id = $v;
			$this->modifiedColumns[] = PersonRolePeer::ROLE_ID;
		}
	...
	public function setPriority($v)
	{
		...
		if ($this->priority !== $v) {
			$this->priority = $v;
			$this->modifiedColumns[] = PersonRolePeer::PRIORITY;
		}
	...
	public function hydrate($row, $startcol = 0, $rehydrate = false)
	{
		try {
			...
			return $startcol + 3; // 3 = PersonRolePeer::NUM_COLUMNS - PersonRolePeer::NUM_LAZY_LOAD_COLUMNS).
    ...
    foreach (sfMixer::getCallables('BasePersonRole:delete:pre') as $callable)
    {
      ...
      if ($ret)
      {
		...
		try {
			PersonRolePeer::doDelete($this, $con);
    ...
    foreach (sfMixer::getCallables('BasePersonRole:delete:post') as $callable)
    {
    ...
    foreach (sfMixer::getCallables('BasePersonRole:save:pre') as $callable)
    {
      ...
      if (is_int($affectedRows))
      {
		...
		try {
			...
			$con->commit();
    foreach (sfMixer::getCallables('BasePersonRole:save:post') as $callable)
    {
	...
	protected function doSave(PropelPDO $con)
	{
		...
		if (!$this->alreadyInSave) {
			...
			if ($this->isModified()) {
				if ($this->isNew()) {
					$pk = PersonRolePeer::doInsert($this, $con);
					...
					$affectedRows += PersonRolePeer::doUpdate($this, $con);
				}
	...
	public function buildCriteria()
	{
		...
		if ($this->isColumnModified(PersonRolePeer::PERSON_ID)) $criteria->add(PersonRolePeer::PERSON_ID, $this->person_id);
		if ($this->isColumnModified(PersonRolePeer::ROLE_ID)) $criteria->add(PersonRolePeer::ROLE_ID, $this->role_id);
		if ($this->isColumnModified(PersonRolePeer::PRIORITY)) $criteria->add(PersonRolePeer::PRIORITY, $this->priority);
	...
	public function buildPkeyCriteria()
	{
		...
		$criteria->add(PersonRolePeer::PERSON_ID, $this->person_id);
		$criteria->add(PersonRolePeer::ROLE_ID, $this->role_id);
	...
	public function getPerson(PropelPDO $con = null)
	{
		if ($this->aPerson === null && ($this->person_id !== null)) {
			...
			/* The following can be used additionally to
			   ...
			   $this->aPerson->addPersonRoles($this);
			 */
		}
		return $this->aPerson;
	}
	...
	public function getRole(PropelPDO $con = null)
	{
		if ($this->aRole === null && ($this->role_id !== null)) {
			...
			/* The following can be used additionally to
			   ...
			   undesirable since it could result in an only partially populated collection
			   in the referenced object.
			   $this->aRole->addPersonRoles($this);
			 */
		}
		return $this->aRole;
	}
  ...
  {
    if (!$callable = sfMixer::getCallable('BasePersonRole:'.$method))
    {
      throw new sfException(sprintf('Call to undefined method BasePersonRole::%s', $method));
    }
