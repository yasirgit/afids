...
/**
 * Base class that represents a row from the 'vocation_class' table.
 *
 * 
 *
 ...
 */
abstract class BaseVocationClass extends BaseObject  implements Persistent {


  const PEER = 'VocationClassPeer';

	/**
	 * The Peer class.
	 * Instance provides a convenient way of calling static methods on a class
	 * that calling code may not be able to identify.
	 * @var        VocationClassPeer
	 */
	...
	/**
	 * Initializes internal state of BaseVocationClass object.
	 * @see        applyDefaults()
	 */
	 ...
	 * @return     VocationClass The current object (for fluent API support)
	 */
	public function setId($v)
	{
		...
		if ($this->id !== $v) {
			$this->id = $v;
			$this->modifiedColumns[] = VocationClassPeer::ID;
		}
	...
	public function setName($v)
	{
		...
		if ($this->name !== $v) {
			$this->name = $v;
			$this->modifiedColumns[] = VocationClassPeer::NAME;
		}
	...
	public function hydrate($row, $startcol = 0, $rehydrate = false)
	{
		try {
			...
			return $startcol + 2; // 2 = VocationClassPeer::NUM_COLUMNS - VocationClassPeer::NUM_LAZY_LOAD_COLUMNS).

		} catch (Exception $e) {
			throw new PropelException("Error populating VocationClass object", $e);
		}
	}
	...
	public function reload($deep = false, PropelPDO $con = null)
	{
		...
		$stmt = VocationClassPeer::doSelectStmt($this->buildPkeyCriteria(), $con);
    ...
    foreach (sfMixer::getCallables('BaseVocationClass:delete:pre') as $callable)
    {
      ...
      if ($ret)
      {
		...
		try {
			VocationClassPeer::doDelete($this, $con);
    ...
    foreach (sfMixer::getCallables('BaseVocationClass:delete:post') as $callable)
    {
    ...
    foreach (sfMixer::getCallables('BaseVocationClass:save:pre') as $callable)
    {
      ...
      if (is_int($affectedRows))
      {
		...
		try {
			...
			$con->commit();
    foreach (sfMixer::getCallables('BaseVocationClass:save:post') as $callable)
    {
      call_user_func($callable, $this, $con, $affectedRows);
    }

			VocationClassPeer::addInstanceToPool($this);
			return $affectedRows;
		} catch (PropelException $e) {
			$con->rollBack();
			throw $e;
	...
	protected function doSave(PropelPDO $con)
	{
		...
		if (!$this->alreadyInSave) {
			...
			if ($this->isModified()) {
				if ($this->isNew()) {
					$pk = VocationClassPeer::doInsert($this, $con);
					...
					$affectedRows += VocationClassPeer::doUpdate($this, $con);
				}
	...
	protected function doValidate($columns = null)
	{
		if (!$this->alreadyInValidation) {
			...
			if (($retval = VocationClassPeer::doValidate($this, $columns)) !== true) {
	...
	{
		$pos = VocationClassPeer::translateFieldName($name, $type, BasePeer::TYPE_NUM);
	...
	{
		$keys = VocationClassPeer::getFieldNames($keyType);
		$result = array(
			$keys[0] => $this->getId(),
			$keys[1] => $this->getName(),
		);
	...
	public function buildCriteria()
	{
		...
		if ($this->isColumnModified(VocationClassPeer::ID)) $criteria->add(VocationClassPeer::ID, $this->id);
		if ($this->isColumnModified(VocationClassPeer::NAME)) $criteria->add(VocationClassPeer::NAME, $this->name);
	...
	public function buildPkeyCriteria()
	{
		...
		$criteria->add(VocationClassPeer::ID, $this->id);
	 ...
	 * objects.
	 *
	 * @param      object $copyObj An object of VocationClass (or compatible) type.
	 ...
	 *
	 * @param      boolean $deepCopy Whether to also copy all rows that refer (by fkey) to the current row.
	 * @return     VocationClass Clone of current object.
	 * @throws     PropelException
	 */
	 ...
	 *
	 * @return     VocationClassPeer
	 */
	public function getPeer()
	{
		if (self::$peer === null) {
			self::$peer = new VocationClassPeer();
		}
		return self::$peer;
	}
	 ...
	 * Otherwise if this VocationClass is new, it will return
	 * an empty collection; or if this VocationClass has previously
	 * been saved, it will retrieve related Applications from storage.
	 *
	 * This method is protected by default in order to keep the public
	 * api reasonable.  You can provide public methods for those you
	 * actually need in VocationClass.
	 */
  ...
  {
    if (!$callable = sfMixer::getCallable('BaseVocationClass:'.$method))
    {
      throw new sfException(sprintf('Call to undefined method BaseVocationClass::%s', $method));
    }
