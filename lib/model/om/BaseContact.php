...
abstract class BaseContact extends BaseObject  implements Persistent {
  ...
  const PEER = 'ContactPeer';
	...
	/**
	 * The value for the letter_sent field.
	 * @var        string
	 */
	protected $letter_sent;
	...
	/**
	 * Get the [optionally formatted] temporal [letter_sent] column value.
	 * 
	 *
	 ...
	 */
	public function getLetterSent($format = 'Y-m-d')
	{
		if ($this->letter_sent === null) {
			return null;
		}


		if ($this->letter_sent === '0000-00-00') {
			// while technically this is not a default value of NULL,
			// this seems to be closest in meaning.
			return null;
		} else {
			try {
				$dt = new DateTime($this->letter_sent);
			} catch (Exception $x) {
				throw new PropelException("Internally stored date/time/timestamp value could not be converted to DateTime: " . var_export($this->letter_sent, true), $x);
			}
		}
	...
	public function setPersonId($v)
	{
		...
		if ($this->person_id !== $v) {
			$this->person_id = $v;
			$this->modifiedColumns[] = ContactPeer::PERSON_ID;
		}
	...
	public function setRefSourceId($v)
	{
		...
		if ($this->ref_source_id !== $v) {
			...
			$this->modifiedColumns[] = ContactPeer::REF_SOURCE_ID;
		}
	...
	public function setSendAppFormat($v)
	{
		...
		if ($this->send_app_format !== $v) {
			...
			$this->modifiedColumns[] = ContactPeer::SEND_APP_FORMAT;
		}
	...
	/**
	 * Sets the value of [letter_sent] column to a normalized version of the date/time value specified.
	 * 
	 * @param      mixed $v string, integer (timestamp), or DateTime value.  Empty string will
	 *						be treated as NULL for temporal objects.
	 * @return     Contact The current object (for fluent API support)
	 */
	public function setLetterSent($v)
	{
		...
		if ( $this->letter_sent !== null || $dt !== null ) {
			// (nested ifs are a little easier to read in this case)

			$currNorm = ($this->letter_sent !== null && $tmpDt = new DateTime($this->letter_sent)) ? $tmpDt->format('Y-m-d') : null;
			$newNorm = ($dt !== null) ? $dt->format('Y-m-d') : null;

			if ( ($currNorm !== $newNorm) // normalized values don't match 
					)
			{
				$this->letter_sent = ($dt ? $dt->format('Y-m-d') : null);
				$this->modifiedColumns[] = ContactPeer::LETTER_SENT;
			}
	...
	public function setContactTypeId($v)
	{
		...
		if ($this->contact_type_id !== $v) {
			...
			$this->modifiedColumns[] = ContactPeer::CONTACT_TYPE_ID;
		}

		if ($this->aContactType !== null && $this->aContactType->getId() !== $v) {
			$this->aContactType = null;
		}
	...
	public function setDateAdded($v)
	{
		...
		if ( $this->date_added !== null || $dt !== null ) {
					...
					)
			{
				...
				$this->modifiedColumns[] = ContactPeer::DATE_ADDED;
			}
		} // if either are not null
	...
	public function setDateUpdated($v)
	{
		...
		if ( $this->date_updated !== null || $dt !== null ) {
					...
					)
			{
				...
				$this->modifiedColumns[] = ContactPeer::DATE_UPDATED;
			}
		} // if either are not null
	...
	public function hydrate($row, $startcol = 0, $rehydrate = false)
	{
		try {
			...
			$this->ref_source_id = ($row[$startcol + 2] !== null) ? (int) $row[$startcol + 2] : null;
			$this->send_app_format = ($row[$startcol + 3] !== null) ? (string) $row[$startcol + 3] : null;
			$this->comment = ($row[$startcol + 4] !== null) ? (string) $row[$startcol + 4] : null;
			$this->letter_sent = ($row[$startcol + 5] !== null) ? (string) $row[$startcol + 5] : null;
			$this->contact_type_id = ($row[$startcol + 6] !== null) ? (int) $row[$startcol + 6] : null;
			$this->date_added = ($row[$startcol + 7] !== null) ? (string) $row[$startcol + 7] : null;
			$this->date_updated = ($row[$startcol + 8] !== null) ? (string) $row[$startcol + 8] : null;
	...
	public function ensureConsistency()
	{
		...
		if ($this->aRefSource !== null && $this->ref_source_id !== $this->aRefSource->getId()) {
			$this->aRefSource = null;
		}
		if ($this->aContactType !== null && $this->contact_type_id !== $this->aContactType->getId()) {
			$this->aContactType = null;
		}
	} // ensureConsistency
	...
	protected function doSave(PropelPDO $con)
	{
		...
		if (!$this->alreadyInSave) {
			...
			if ($this->aContactType !== null) {
				if ($this->aContactType->isModified() || $this->aContactType->isNew()) {
					$affectedRows += $this->aContactType->save($con);
				}
				$this->setContactType($this->aContactType);
			}
	...
	public function toArray($keyType = BasePeer::TYPE_PHPNAME, $includeLazyLoadColumns = true)
	{
		...
		$result = array(
			...
			$keys[2] => $this->getRefSourceId(),
			$keys[3] => $this->getSendAppFormat(),
			$keys[4] => $this->getComment(),
			$keys[5] => $this->getLetterSent(),
			$keys[6] => $this->getContactTypeId(),
			$keys[7] => $this->getDateAdded(),
			$keys[8] => $this->getDateUpdated(),
	...
	public function fromArray($arr, $keyType = BasePeer::TYPE_PHPNAME)
	{
		...
		if (array_key_exists($keys[2], $arr)) $this->setRefSourceId($arr[$keys[2]]);
		if (array_key_exists($keys[3], $arr)) $this->setSendAppFormat($arr[$keys[3]]);
		if (array_key_exists($keys[4], $arr)) $this->setComment($arr[$keys[4]]);
		if (array_key_exists($keys[5], $arr)) $this->setLetterSent($arr[$keys[5]]);
		if (array_key_exists($keys[6], $arr)) $this->setContactTypeId($arr[$keys[6]]);
		if (array_key_exists($keys[7], $arr)) $this->setDateAdded($arr[$keys[7]]);
		if (array_key_exists($keys[8], $arr)) $this->setDateUpdated($arr[$keys[8]]);
	...
	public function buildCriteria()
	{
		...
		if ($this->isColumnModified(ContactPeer::PERSON_ID)) $criteria->add(ContactPeer::PERSON_ID, $this->person_id);
		if ($this->isColumnModified(ContactPeer::REF_SOURCE_ID)) $criteria->add(ContactPeer::REF_SOURCE_ID, $this->ref_source_id);
		if ($this->isColumnModified(ContactPeer::SEND_APP_FORMAT)) $criteria->add(ContactPeer::SEND_APP_FORMAT, $this->send_app_format);
		if ($this->isColumnModified(ContactPeer::COMMENT)) $criteria->add(ContactPeer::COMMENT, $this->comment);
		if ($this->isColumnModified(ContactPeer::LETTER_SENT)) $criteria->add(ContactPeer::LETTER_SENT, $this->letter_sent);
		if ($this->isColumnModified(ContactPeer::CONTACT_TYPE_ID)) $criteria->add(ContactPeer::CONTACT_TYPE_ID, $this->contact_type_id);
		if ($this->isColumnModified(ContactPeer::DATE_ADDED)) $criteria->add(ContactPeer::DATE_ADDED, $this->date_added);
		if ($this->isColumnModified(ContactPeer::DATE_UPDATED)) $criteria->add(ContactPeer::DATE_UPDATED, $this->date_updated);
	...
	public function copyInto($copyObj, $deepCopy = false)
	{
		...
		$copyObj->setLetterSent($this->letter_sent);
	...
	{
		if ($v === null) {
			$this->setRefSourceId(NULL);
		} else {
			$this->setRefSourceId($v->getId());
		}
	...
	/**
	 * Declares an association between this object and a ContactType object.
	 *
	 * @param      ContactType $v
	 * @return     Contact The current object (for fluent API support)
	 * @throws     PropelException
	 */
	public function setContactType(ContactType $v = null)
	{
		if ($v === null) {
		...
		// Add binding for other direction of this n:n relationship.
		// If this object has already been added to the ContactType object, it will not be re-added.
		if ($v !== null) {
			$v->addContact($this);
		}
