...
/**
 * Base class that represents a row from the 'ref_source' table.
 *
 * 
 *
 ...
 */
abstract class BaseRefSource extends BaseObject  implements Persistent {


  const PEER = 'RefSourcePeer';
	...
	/**
	 * Initializes internal state of BaseRefSource object.
	 * @see        applyDefaults()
	 */
	 ...
	 * @return     RefSource The current object (for fluent API support)
	 */
	public function setId($v)
	{
		...
		if ($this->id !== $v) {
			$this->id = $v;
			$this->modifiedColumns[] = RefSourcePeer::ID;
		}
	...
	public function setSourceName($v)
	{
		...
		if ($this->source_name !== $v) {
			$this->source_name = $v;
			$this->modifiedColumns[] = RefSourcePeer::SOURCE_NAME;
		}
	...
	public function hydrate($row, $startcol = 0, $rehydrate = false)
	{
		try {
			...
			$this->source_name = ($row[$startcol + 1] !== null) ? (string) $row[$startcol + 1] : null;
			$this->resetModified();
			...
			return $startcol + 2; // 2 = RefSourcePeer::NUM_COLUMNS - RefSourcePeer::NUM_LAZY_LOAD_COLUMNS).

		} catch (Exception $e) {
			throw new PropelException("Error populating RefSource object", $e);
		}
	}
	...
	public function reload($deep = false, PropelPDO $con = null)
	{
		...
		$stmt = RefSourcePeer::doSelectStmt($this->buildPkeyCriteria(), $con);
    ...
    foreach (sfMixer::getCallables('BaseRefSource:delete:pre') as $callable)
    {
      ...
      if ($ret)
      {
		...
		try {
			RefSourcePeer::doDelete($this, $con);
    ...
    foreach (sfMixer::getCallables('BaseRefSource:delete:post') as $callable)
    {
    ...
    foreach (sfMixer::getCallables('BaseRefSource:save:pre') as $callable)
    {
      ...
      if (is_int($affectedRows))
      {
		...
		try {
			...
			$con->commit();
    foreach (sfMixer::getCallables('BaseRefSource:save:post') as $callable)
    {
      call_user_func($callable, $this, $con, $affectedRows);
    }

			RefSourcePeer::addInstanceToPool($this);
			return $affectedRows;
		} catch (PropelException $e) {
			$con->rollBack();
			throw $e;
	...
	protected function doSave(PropelPDO $con)
	{
		...
		if (!$this->alreadyInSave) {
			...
			if ($this->isModified()) {
				if ($this->isNew()) {
					$pk = RefSourcePeer::doInsert($this, $con);
					...
					$affectedRows += RefSourcePeer::doUpdate($this, $con);
				}
	...
	protected function doValidate($columns = null)
	{
		if (!$this->alreadyInValidation) {
			...
			if (($retval = RefSourcePeer::doValidate($this, $columns)) !== true) {
	...
	{
		$pos = RefSourcePeer::translateFieldName($name, $type, BasePeer::TYPE_NUM);
	...
	public function fromArray($arr, $keyType = BasePeer::TYPE_PHPNAME)
	{
		...
		if (array_key_exists($keys[1], $arr)) $this->setSourceName($arr[$keys[1]]);
	}
	...
	public function buildCriteria()
	{
		...
		if ($this->isColumnModified(RefSourcePeer::ID)) $criteria->add(RefSourcePeer::ID, $this->id);
		if ($this->isColumnModified(RefSourcePeer::SOURCE_NAME)) $criteria->add(RefSourcePeer::SOURCE_NAME, $this->source_name);
	...
	public function buildPkeyCriteria()
	{
		...
		$criteria->add(RefSourcePeer::ID, $this->id);
	 ...
	 * objects.
	 *
	 * @param      object $copyObj An object of RefSource (or compatible) type.
	 ...
	 *
	 * @param      boolean $deepCopy Whether to also copy all rows that refer (by fkey) to the current row.
	 * @return     RefSource Clone of current object.
	 * @throws     PropelException
	 */
	 ...
	 *
	 * @return     RefSourcePeer
	 */
	public function getPeer()
	{
		if (self::$peer === null) {
			self::$peer = new RefSourcePeer();
		}
		return self::$peer;
	}
	 ...
	 * Otherwise if this RefSource has previously been saved, it will retrieve
	 * related Applications from storage. If this RefSource is new, it will return
		...
		if ($this->collApplications === null) {
			if ($this->isNew()) {
			   $this->collApplications = array();
				...
				$criteria->add(ApplicationPeer::REFERRAL_SOURCE, $this->id);
	 ...
	 * Otherwise if this RefSource is new, it will return
	 * an empty collection; or if this RefSource has previously
	 * been saved, it will retrieve related Applications from storage.
	 *
	 * This method is protected by default in order to keep the public
	 * api reasonable.  You can provide public methods for those you
	 * actually need in RefSource.
	 */
	public function getApplicationsJoinMember($criteria = null, $con = null, $join_behavior = Criteria::LEFT_JOIN)
	{
		if ($criteria === null) {
			$criteria = new Criteria(RefSourcePeer::DATABASE_NAME);
		}
		...
		if ($this->collApplications === null) {
			if ($this->isNew()) {
				...
				$this->collApplications = ApplicationPeer::doSelectJoinMember($criteria, $con, $join_behavior);
			}
		} else {
			// the following code is to determine if a new query is
			// called for.  If the criteria is the same as the last
	 ...
	 * If this collection has already been initialized with an identical Criteria, it returns the collection.
	 * Otherwise if this RefSource has previously been saved, it will retrieve
	 * related Contacts from storage. If this RefSource is new, it will return
	 * an empty collection or the current collection, the criteria is ignored on a new object.
	 *
	 * @param      PropelPDO $con
	 * @param      Criteria $criteria
		...
		if ($this->collContacts === null) {
			if ($this->isNew()) {
			   $this->collContacts = array();
				...
				$criteria->add(ContactPeer::REF_SOURCE_ID, $this->id);
  ...
  {
    if (!$callable = sfMixer::getCallable('BaseRefSource:'.$method))
    {
      throw new sfException(sprintf('Call to undefined method BaseRefSource::%s', $method));
    }
