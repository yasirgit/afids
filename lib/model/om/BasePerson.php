...
abstract class BasePerson extends BaseObject  implements Persistent {
  ...
  const PEER = 'PersonPeer';
	...
	/**
	 * @var        array WingLeader[] Collection to store aggregation of WingLeader objects.
	 */
	protected $collWingLeaders;

	/**
	 * @var        Criteria The criteria used to select the current contents of collWingLeaders.
	 */
	private $lastWingLeaderCriteria = null;
	...
	protected function doSave(PropelPDO $con)
	{
		...
		if (!$this->alreadyInSave) {
			...
			if ($this->collWingLeaders !== null) {
				foreach ($this->collWingLeaders as $referrerFK) {
					if (!$referrerFK->isDeleted()) {
						$affectedRows += $referrerFK->save($con);
					}
				}
	...
	/**
	 * Clears out the collWingLeaders collection (array).
	 *
	 * This does not modify the database; however, it will remove any associated objects, causing
	 * them to be refetched by subsequent calls to accessor method.
	 *
	 ...
	 * @see        addWingLeaders()
	 */
	public function clearWingLeaders()
	{
		$this->collWingLeaders = null; // important to set this to NULL since that means it is uninitialized
	}

	/**
	 * Initializes the collWingLeaders collection (array).
	 *
	 * By default this just sets the collWingLeaders collection to an empty array (like clearcollWingLeaders());
	 * however, you may wish to override this method in your stub class to provide setting appropriate
	 * to your application -- for example, setting the initial array to the values stored in database.
	 *
	 * @return     void
	 */
	public function initWingLeaders()
	{
		$this->collWingLeaders = array();
	}

	/**
	 * Gets an array of WingLeader objects which contain a foreign key that references this object.
	 *
	 * If this collection has already been initialized with an identical Criteria, it returns the collection.
	 * Otherwise if this Person has previously been saved, it will retrieve
	 * related WingLeaders from storage. If this Person is new, it will return
	 * an empty collection or the current collection, the criteria is ignored on a new object.
	 *
	 * @param      PropelPDO $con
	 * @param      Criteria $criteria
	 * @return     array WingLeader[]
	 * @throws     PropelException
	 */
	public function getWingLeaders($criteria = null, PropelPDO $con = null)
	{
		if ($criteria === null) {
			$criteria = new Criteria(PersonPeer::DATABASE_NAME);
		}
		...
		if ($this->collWingLeaders === null) {
			if ($this->isNew()) {
			   $this->collWingLeaders = array();
				...
				$criteria->add(WingLeaderPeer::PERSON_ID, $this->id);

				WingLeaderPeer::addSelectColumns($criteria);
				$this->collWingLeaders = WingLeaderPeer::doSelect($criteria, $con);
			}
		} else {
			// criteria has no effect for a new object
			if (!$this->isNew()) {
				...
				WingLeaderPeer::addSelectColumns($criteria);
				if (!isset($this->lastWingLeaderCriteria) || !$this->lastWingLeaderCriteria->equals($criteria)) {
					$this->collWingLeaders = WingLeaderPeer::doSelect($criteria, $con);
				}
			}
		}
		$this->lastWingLeaderCriteria = $criteria;
		return $this->collWingLeaders;
	}

	/**
	 * Returns the number of related WingLeader objects.
	 *
	 * @param      Criteria $criteria
	 * @param      boolean $distinct
	 * @param      PropelPDO $con
	 * @return     int Count of related WingLeader objects.
	 * @throws     PropelException
	 */
	public function countWingLeaders(Criteria $criteria = null, $distinct = false, PropelPDO $con = null)
	{
		if ($criteria === null) {
			$criteria = new Criteria(PersonPeer::DATABASE_NAME);
		} else {
		...
		if ($this->collWingLeaders === null) {
			if ($this->isNew()) {
				...
				$count = WingLeaderPeer::doCount($criteria, $con);
			}
		} else {
			// criteria has no effect for a new object
			if (!$this->isNew()) {
				...
				if (!isset($this->lastWingLeaderCriteria) || !$this->lastWingLeaderCriteria->equals($criteria)) {
					...
					$count = count($this->collWingLeaders);
				}
			} else {
				$count = count($this->collWingLeaders);
			}
	...
	/**
	 * Method called to associate a WingLeader object to this object
	 * through the WingLeader foreign key attribute.
	 *
	 * @param      WingLeader $l WingLeader
	 * @return     void
	 * @throws     PropelException
	 */
	public function addWingLeader(WingLeader $l)
	{
		if ($this->collWingLeaders === null) {
			$this->initWingLeaders();
		}
		if (!in_array($l, $this->collWingLeaders, true)) { // only add it if the **same** object is not already associated
			array_push($this->collWingLeaders, $l);
			$l->setPerson($this);
		}
	}
	...
	public function clearAllReferences($deep = false)
	{
		if ($deep) {
			...
			if ($this->collWingLeaders) {
				foreach ((array) $this->collWingLeaders as $o) {
					$o->clearAllReferences($deep);
				}
			}
		} // if ($deep)
