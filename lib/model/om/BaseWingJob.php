...
/**
 * Base class that represents a row from the 'wing_job' table.
 *
 * 
 *
 ...
 */
abstract class BaseWingJob extends BaseObject  implements Persistent {


  const PEER = 'WingJobPeer';
	...
	/**
	 * Initializes internal state of BaseWingJob object.
	 * @see        applyDefaults()
	 */
	 ...
	 * @return     WingJob The current object (for fluent API support)
	 */
	public function setId($v)
	{
		...
		if ($this->id !== $v) {
			$this->id = $v;
			$this->modifiedColumns[] = WingJobPeer::ID;
		}
	...
	public function setName($v)
	{
		...
		if ($this->name !== $v) {
			$this->name = $v;
			$this->modifiedColumns[] = WingJobPeer::NAME;
		}
	...
	public function setShortName($v)
	{
		...
		if ($this->short_name !== $v) {
			$this->short_name = $v;
			$this->modifiedColumns[] = WingJobPeer::SHORT_NAME;
		}
	...
	public function setPersonId($v)
	{
		...
		if ($this->person_id !== $v) {
			$this->person_id = $v;
			$this->modifiedColumns[] = WingJobPeer::PERSON_ID;
		}
	...
	public function hydrate($row, $startcol = 0, $rehydrate = false)
	{
		try {
			...
			return $startcol + 4; // 4 = WingJobPeer::NUM_COLUMNS - WingJobPeer::NUM_LAZY_LOAD_COLUMNS).

		} catch (Exception $e) {
			throw new PropelException("Error populating WingJob object", $e);
		}
	}
	...
	public function reload($deep = false, PropelPDO $con = null)
	{
		...
		$stmt = WingJobPeer::doSelectStmt($this->buildPkeyCriteria(), $con);
    ...
    foreach (sfMixer::getCallables('BaseWingJob:delete:pre') as $callable)
    {
    ...
    foreach (sfMixer::getCallables('BaseWingJob:delete:post') as $callable)
    {
    ...
    foreach (sfMixer::getCallables('BaseWingJob:save:pre') as $callable)
    {
      ...
      if (is_int($affectedRows))
      {
		...
		try {
			...
			$con->commit();
    foreach (sfMixer::getCallables('BaseWingJob:save:post') as $callable)
    {
      call_user_func($callable, $this, $con, $affectedRows);
    }

			WingJobPeer::addInstanceToPool($this);
			return $affectedRows;
		} catch (PropelException $e) {
			$con->rollBack();
			throw $e;
	...
	protected function doSave(PropelPDO $con)
	{
		...
		if (!$this->alreadyInSave) {
			...
			if ($this->isModified()) {
				if ($this->isNew()) {
					$pk = WingJobPeer::doInsert($this, $con);
					...
					$affectedRows += WingJobPeer::doUpdate($this, $con);
				}
	...
	protected function doValidate($columns = null)
	{
		if (!$this->alreadyInValidation) {
			...
			if (($retval = WingJobPeer::doValidate($this, $columns)) !== true) {
	...
	{
		$pos = WingJobPeer::translateFieldName($name, $type, BasePeer::TYPE_NUM);
	...
	{
		$keys = WingJobPeer::getFieldNames($keyType);
		$result = array(
			$keys[0] => $this->getId(),
			$keys[1] => $this->getName(),
	...
	public function buildCriteria()
	{
		...
		if ($this->isColumnModified(WingJobPeer::ID)) $criteria->add(WingJobPeer::ID, $this->id);
		if ($this->isColumnModified(WingJobPeer::NAME)) $criteria->add(WingJobPeer::NAME, $this->name);
		if ($this->isColumnModified(WingJobPeer::SHORT_NAME)) $criteria->add(WingJobPeer::SHORT_NAME, $this->short_name);
		if ($this->isColumnModified(WingJobPeer::PERSON_ID)) $criteria->add(WingJobPeer::PERSON_ID, $this->person_id);
	...
	public function buildPkeyCriteria()
	{
		...
		$criteria->add(WingJobPeer::ID, $this->id);
	 ...
	 * objects.
	 *
	 * @param      object $copyObj An object of WingJob (or compatible) type.
	 ...
	 *
	 * @param      boolean $deepCopy Whether to also copy all rows that refer (by fkey) to the current row.
	 * @return     WingJob Clone of current object.
	 * @throws     PropelException
	 */
	 ...
	 * Otherwise if this WingJob has previously been saved, it will retrieve
	 * related MemberWingJobs from storage. If this WingJob is new, it will return
		...
		if ($this->collMemberWingJobs === null) {
			if ($this->isNew()) {
			   $this->collMemberWingJobs = array();
				...
				$criteria->add(MemberWingJobPeer::WING_JOB_ID, $this->id);
	 ...
	 * Otherwise if this WingJob is new, it will return
	 * an empty collection; or if this WingJob has previously
	 * been saved, it will retrieve related MemberWingJobs from storage.
	 *
	 * This method is protected by default in order to keep the public
	 * api reasonable.  You can provide public methods for those you
	 * actually need in WingJob.
	 */
	public function getMemberWingJobsJoinMember($criteria = null, $con = null, $join_behavior = Criteria::LEFT_JOIN)
	{
		if ($criteria === null) {
			$criteria = new Criteria(WingJobPeer::DATABASE_NAME);
		}
		...
		if ($this->collMemberWingJobs === null) {
			if ($this->isNew()) {
				...
				$this->collMemberWingJobs = MemberWingJobPeer::doSelectJoinMember($criteria, $con, $join_behavior);
			}
		} else {
			// the following code is to determine if a new query is
			// called for.  If the criteria is the same as the last
  ...
  {
    if (!$callable = sfMixer::getCallable('BaseWingJob:'.$method))
    {
      throw new sfException(sprintf('Call to undefined method BaseWingJob::%s', $method));
    }
