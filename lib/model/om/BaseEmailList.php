...
abstract class BaseEmailList extends BaseObject  implements Persistent {
  ...
  const PEER = 'EmailListPeer';
	...
	/**
	 * Initializes internal state of BaseEmailList object.
	 * @see        applyDefaults()
	 */
	...
	public function setDescription($v)
	{
		...
		if ($this->description !== $v) {
			$this->description = $v;
			$this->modifiedColumns[] = EmailListPeer::DESCRIPTION;
		}
	...
	public function setIsPrivate($v)
	{
		...
		if ($this->is_private !== $v) {
			$this->is_private = $v;
			$this->modifiedColumns[] = EmailListPeer::IS_PRIVATE;
		}
	...
	public function setWingId($v)
	{
		...
		if ($this->wing_id !== $v) {
			$this->wing_id = $v;
			$this->modifiedColumns[] = EmailListPeer::WING_ID;
		}
	...
	public function hydrate($row, $startcol = 0, $rehydrate = false)
	{
		try {
			...
			$this->is_private = ($row[$startcol + 2] !== null) ? (int) $row[$startcol + 2] : null;
			...
			return $startcol + 4; // 4 = EmailListPeer::NUM_COLUMNS - EmailListPeer::NUM_LAZY_LOAD_COLUMNS).
    ...
    foreach (sfMixer::getCallables('BaseEmailList:delete:pre') as $callable)
    {
      ...
      if ($ret)
      {
		...
		try {
			EmailListPeer::doDelete($this, $con);
    ...
    foreach (sfMixer::getCallables('BaseEmailList:delete:post') as $callable)
    {
    ...
    foreach (sfMixer::getCallables('BaseEmailList:save:pre') as $callable)
    {
      ...
      if (is_int($affectedRows))
      {
		...
		try {
			...
			$con->commit();
    foreach (sfMixer::getCallables('BaseEmailList:save:post') as $callable)
    {
	...
	protected function doSave(PropelPDO $con)
	{
		...
		if (!$this->alreadyInSave) {
			...
			if ($this->isModified()) {
				if ($this->isNew()) {
					$pk = EmailListPeer::doInsert($this, $con);
	...
	public function buildCriteria()
	{
		...
		if ($this->isColumnModified(EmailListPeer::DESCRIPTION)) $criteria->add(EmailListPeer::DESCRIPTION, $this->description);
		if ($this->isColumnModified(EmailListPeer::IS_PRIVATE)) $criteria->add(EmailListPeer::IS_PRIVATE, $this->is_private);
		if ($this->isColumnModified(EmailListPeer::WING_ID)) $criteria->add(EmailListPeer::WING_ID, $this->wing_id);
	 ...
	 * related EmailListPersons from storage. If this EmailList is new, it will return
	 ...
	 * Otherwise if this EmailList is new, it will return
	 * an empty collection; or if this EmailList has previously
	 * been saved, it will retrieve related EmailListPersons from storage.
	 *
	 * This method is protected by default in order to keep the public
	 * api reasonable.  You can provide public methods for those you
	 * actually need in EmailList.
	 */
	public function getEmailListPersonsJoinPerson($criteria = null, $con = null, $join_behavior = Criteria::LEFT_JOIN)
	{
		if ($criteria === null) {
			$criteria = new Criteria(EmailListPeer::DATABASE_NAME);
		}
		...
		if ($this->collEmailListPersons === null) {
			if ($this->isNew()) {
				...
				$this->collEmailListPersons = EmailListPersonPeer::doSelectJoinPerson($criteria, $con, $join_behavior);
			}
		} else {
			// the following code is to determine if a new query is
			// called for.  If the criteria is the same as the last
	 ...
	 * actually need in EmailList.
	 */
	public function getEmailListRolesJoinRole($criteria = null, $con = null, $join_behavior = Criteria::LEFT_JOIN)
	{
		if ($criteria === null) {
			$criteria = new Criteria(EmailListPeer::DATABASE_NAME);
		}
		...
		if ($this->collEmailListRoles === null) {
			if ($this->isNew()) {
				...
				$this->collEmailListRoles = EmailListRolePeer::doSelectJoinRole($criteria, $con, $join_behavior);
			}
		} else {
			// the following code is to determine if a new query is
			// called for.  If the criteria is the same as the last
  ...
  {
    if (!$callable = sfMixer::getCallable('BaseEmailList:'.$method))
    {
      throw new sfException(sprintf('Call to undefined method BaseEmailList::%s', $method));
    }
